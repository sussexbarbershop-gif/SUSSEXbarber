...
--
-- Historical phone_key values are suffixes; new ones use e164: plus the full
-- international number. For new bookings, "06 1234 5678", "+31 6 1234 5678"
-- and "0031612345678" are one customer. Legacy profiles are not inferred or merged.
--
...
CREATE TABLE IF NOT EXISTS bookings (
    ...
    -- they had to agree.
    -- New bookings only: old phone values and customer links are preserved.
    phone_e164    text,
...
-- Legacy customer links and raw numbers remain unchanged; no identity backfill.
CREATE INDEX IF NOT EXISTS bookings_by_customer ON bookings (customer_id);

-- Additive international identity. Do not infer/backfill legacy country codes.
ALTER TABLE bookings ADD COLUMN IF NOT EXISTS phone_e164 text;
CREATE INDEX IF NOT EXISTS bookings_by_e164 ON bookings (phone_e164, booked_on);
