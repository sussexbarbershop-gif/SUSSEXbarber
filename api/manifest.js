...
module.exports = async function handler(req, res) {
  ...
  const manifest = {
    ...
    // There was a second shortcut here, to the device report, because an
    // installed app has no address bar and there was no other way to open a
    // diagnostic on the one phone whose answer was needed. It did its job —
    // that report is what proved the missing pixels were outside the window
    // rather than an inset inside it — and then it was a line reading "Device
    // report" in a menu customers open by holding the shop's icon. The report
    // is still there on five taps of the logo, and ?report=1 still opens it.
  };
