...
ok('with the shop\'s number to call',
   /id="bookingClosed"[\s\S]{0,4000}cms-contact-phone-link/.test(site), true);
// It follows the settings like every other number on the page.
ok('which follows the panel too',
   /id="bookingClosed"[\s\S]{0,4600}class="cms-contact-phone"/.test(site), true);
...
// They booked in good faith through a website that was not ready. The shop has
// since cancelled those appointments, so somebody could otherwise turn up to a
// chair nobody was expecting — let down by the website, not by the shop.
ok('the notice apologises to them', /we are sorry/i.test(site), true);

// What a customer reads, with the markup comments taken out: the comments
// explain the wording and use the same words, and it is the reading that
// matters.
const closedNotice = (site.match(/id="bookingClosed"[\s\S]*?<\/a>/) || [''])[0]
  .replace(/<!--[\s\S]*?-->/g, '');

// This said "may not have reached us" while nobody knew what would become of
// those rows, which was honest then. They have been cancelled now, and the old
// wording would be a way of not saying so.
ok('it says the appointments were cancelled',
   /those appointments were cancelled/i.test(closedNotice), true);
// And that an email went out, because one did: the panel's only way to remove
// a booking is cancelBooking, which emails the customer. Being told the same
// thing twice is how somebody knows it is true.
ok('and that an email was sent about it',
   /you should have had an email/i.test(closedNotice), true);
ok('the panel could not have removed one silently',
   /sendCustomerCancellation\(cancelled, config\)/.test(api), true);
// There is nothing to confirm any more, so the action changed with the facts.
ok('it asks them to book again rather than to confirm',
   /book you in properly/i.test(closedNotice), true);
ok('and no longer offers to confirm what is gone',
   /confirm it for you/i.test(closedNotice), false);

// Both languages, like everything else in this notice.
ok('in Dutch as well', /Onze excuses/.test(site), true);
ok('saying the same there', /die afspraken zijn geannuleerd/.test(site), true);
ok('including the email', /daarover heeft u een e-mail gehad/.test(site), true);
