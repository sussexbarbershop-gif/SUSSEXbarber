...
//
// Two separate things fix that, and neither substitutes for the other: icons,
// and a manifest with a short_name.
const fs = require('fs');
...
ok('there is one', fs.existsSync(manifestPath), true);
let m = {};
try { m = JSON.parse(fs.readFileSync(manifestPath, 'utf8')); }
catch (e) { ok('and it parses', e.message, ''); }
ok('the page links to it', /<link rel="manifest" href="\/manifest\.webmanifest">/.test(html), true);
...
ok('and a short one for the icon label', typeof m.short_name === 'string', true);
...
// Two kinds, and the difference is why this is generated by a script.
//
// `any` is drawn as it is. `maskable` is cropped by Android to whatever shape
// the launcher uses — a circle, a squircle, a rounded square — and only the
// middle 80% is guaranteed to survive. An `any` icon used as maskable loses
// its edges; a maskable one used as `any` merely looks generously padded.
const icons = m.icons || [];
...
// Declared is not the same as present, and a manifest naming an icon that
// 404s is how you end up back at the grey square with the letter in it.
icons.forEach(i => {
  ok(`${i.src} exists`, fs.existsSync(path.join(root, i.src.replace(/^\//, ''))), true);
});
...
// still the shop.
['privacy.html', 'terms.html', 'cancel.html', path.join('admin', 'index.html')].forEach(page => {
  const src = fs.readFileSync(path.join(root, page), 'utf8');
  ok(`${page} has the icon too`, /<link rel="icon"/.test(src), true);
});

console.log('--- what the icons actually contain ---');
...
(async () => {
  for (const file of ['icon-192.png', 'icon-maskable-512.png', 'favicon-32.png',
    ...
    ok(`${file} is not transparent`, meta.hasAlpha === false || stats.isOpaque === true, true);
  }

  // The generator is committed, so the icons can be made again from the logo
  // rather than being files nobody knows the origin of.
