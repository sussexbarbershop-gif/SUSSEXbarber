...
ok('Hemen Wed starts 10:00', hemenWed[0], '10:00');
ok('Hemen Wed ends 17:30',   hemenWed[hemenWed.length-1], '17:30');
ok('Hemen Wed skips break',  hemenWed.includes('13:30'), false);
ok('Hemen Wed keeps 13:00',  hemenWed.includes('13:00'), true);
ok('Hemen Wed resumes 14:00',hemenWed.includes('14:00'), true);
...
ok('Raman Mon starts 12:00', ramanMon[0], '12:00');
ok('Raman Mon ends 17:30',   ramanMon[ramanMon.length-1], '17:30');
ok('Raman Mon has no break', ramanMon.includes('13:30'), true);
ok('Raman Mon slot count',   ramanMon.length, 12);
ok('Raman Sat starts 10:00', slotsForDate(D('2026-08-22'))[0], '10:00');
ok('Raman Sat does break',   slotsForDate(D('2026-08-22')).includes('13:30'), false);
...
ok('early closing trims to 14:30', trimmed[trimmed.length-1], '14:30');
...
ok('Any: break still blocked (all share it)',
   slotsForDate(D('2026-08-22')).includes('13:30'), false);
...
// still offered ten that morning.
{
  ...
  ok('nothing before now is offered',   todaySlots.some(s => s === '10:00'), false);
  ok('nor inside the notice window',    todaySlots.some(s => s === '16:00'), false);
  ok('the first offer clears the notice', todaySlots[0], '16:30');
  ok('the rest of the day is intact',   todaySlots, ['16:30', '17:00', '17:30']);

  // Tomorrow is a full day regardless of the hour it is looked at.
  ok('tomorrow starts at opening', slotsForDate(D('2026-08-21'))[0], '10:00');
