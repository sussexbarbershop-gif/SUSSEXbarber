...
ok('and it is nullable', /customer_id\s+integer REFERENCES/.test(schema), true);

console.log('--- a database that predates the table catches up on its own ---');
// The same trap the other four columns had: CREATE TABLE IF NOT EXISTS does
...
ok('customerFor exists', /async function customerFor/.test(dbjs), true);
...
ok('it upserts rather than checking first',
   /INSERT INTO customers[\s\S]{0,300}ON CONFLICT \(phone_key\) DO UPDATE/.test(dbjs), true);
ok('a later booking refreshes the name',
   /name\s*=\s*COALESCE\(NULLIF\(EXCLUDED\.name, ''\)/.test(dbjs), true);
...
ok('but an empty one does not wipe it', /NULLIF\(EXCLUDED\.name, ''\)/.test(dbjs), true);

ok('the booking carries the link', /customer_id\)[\s\S]{0,300}\$\{customerId\}/.test(api), true);
// A number that is not one — the shop typing a booking for a walk-in with no
// phone — must still produce a booking.
ok('no number still books', /if \(!key\) return null/.test(dbjs), true);
